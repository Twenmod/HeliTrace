...
static GLTexture* renderTargetDebug = 0;
static GLTexture* renderTargetLight = 0;
static GLTexture* renderNormals = 0;
static GLTexture* renderPos = 0;
static GLTexture* renderTargetBuffer = 0;
static GLTexture* renderTargetLightBuffer = 0;
static GLTexture* renderNormalsBuffer = 0;
static GLTexture* renderPosBuffer = 0;
static GLTexture* overlayBuffer = 0;
static int scrwidth = 0, scrheight = 0;
static TheApp* app = 0;
uint keystate[256] = {0};

uint secondDofFBO, secondDofFBOTexture;
...
void InitRenderTarget(int w, int h)
{
	...
	renderTargetDebug = new GLTexture(scrwidth, scrheight, GLTexture::FLOAT);
	renderTargetLight = new GLTexture(scrwidth, scrheight, GLTexture::FLOAT);
	renderNormals = new GLTexture(scrwidth, scrheight, GLTexture::FLOAT);
	renderPos = new GLTexture(scrwidth, scrheight, GLTexture::FLOAT);
	renderTargetBuffer = new GLTexture(scrwidth, scrheight, GLTexture::FLOAT);
	renderTargetLightBuffer = new GLTexture(scrwidth, scrheight, GLTexture::FLOAT);
	renderNormalsBuffer = new GLTexture(scrwidth, scrheight, GLTexture::FLOAT);
	renderPosBuffer = new GLTexture(scrwidth, scrheight, GLTexture::FLOAT);
	overlayBuffer = new GLTexture(UI_WIDTH, UI_HEIGHT, GLTexture::FLOAT, GL_NEAREST);
}
...
void ReshapeWindowCallback(GLFWwindow*, int w, int h)
{
	...
	float actualAspect = static_cast<float>(WINDOW_WIDTH) / static_cast<float>(WINDOW_HEIGTH);
	if (actualAspect < ASPECT_RATIO)
	{
		int newHeight = static_cast<int>(static_cast<float>(WINDOW_WIDTH) / ASPECT_RATIO);
		int yOffset = (WINDOW_HEIGTH - newHeight) / 2;
		glViewport(0, yOffset, static_cast<uint>(WINDOW_WIDTH), newHeight);
	}
	else if (actualAspect > ASPECT_RATIO)
	{
		int newWidth = static_cast<int>(WINDOW_HEIGTH * ASPECT_RATIO);
		int xOffset = (WINDOW_WIDTH - newWidth) / 2;
		glViewport(xOffset, 0, newWidth, static_cast<uint>(WINDOW_HEIGTH));
	}
	overlayShader->SetUInt("Flip", true);
	overlayShader->SetInputTexture(0, "c", secondDofFBOTexture);
	overlayShader->SetInputTexture(1, "overlay", overlayBuffer);
	overlayShader->SetInt("resolutionX", RENDER_WIDTH);
	overlayShader->SetInt("resolutionY", RENDER_HEIGHT);
	...
	{
		{
			...
			frameReady.wait(lock, [] { return !frameIsReady; }); // wait for main thread to copy over the surfaces
...
void main()
{
	...
	glfwWindowHint(GLFW_RESIZABLE, GL_TRUE /* harder :) */);
#ifdef FULLSCREEN
	window = glfwCreateWindow(RENDER_WIDTH, RENDER_HEIGHT, "Tmpl8RT", glfwGetPrimaryMonitor(), 0);
#else
#ifdef DOUBLESIZE
	window = glfwCreateWindow(RENDER_WIDTH * 2, RENDER_HEIGHT * 2, "Tmpl8RT", 0, 0);
#else
	window = glfwCreateWindow(WINDOW_WIDTH, WINDOW_HEIGTH, "Tmpl8RT", 0, 0);
#endif
#endif
	...
	GLFWimage images;
	images.pixels = stbi_load("../assets/helitrace.png", &images.width, &images.height, nullptr, 4);
	...
	InitRenderTarget(RENDER_WIDTH, RENDER_HEIGHT);
	auto screen = new FloatTexture(RENDER_WIDTH, RENDER_HEIGHT);
	auto debugScreen = new FloatTexture(RENDER_WIDTH, RENDER_HEIGHT);
	auto screenLights = new FloatTexture(RENDER_WIDTH, RENDER_HEIGHT);
	auto normals = new FloatTexture(RENDER_WIDTH, RENDER_HEIGHT);
	auto positions = new FloatTexture(RENDER_WIDTH, RENDER_HEIGHT);
	auto overlay = new FloatTexture(UI_WIDTH, UI_HEIGHT);
	overlay->Clear(float4(0.f, 0.f, 0.f, 0.f));
	app = new Renderer();
	app->screen = screen;
	app->debugScreen = debugScreen;
	app->normals = normals;
	app->screenLighting = screenLights;
...
#pragma region FBO and shader Setup

	auto blurShader = new Shader("Blur.vert",
	                             "Blur.frag", false);

	auto dofShader = new Shader("Blur.vert",
	                            "Dof.frag", false);


	auto copyShader = new Shader("Copy.vert",
	                             "Copy.frag", false);

	auto reprojectionShader = new Shader("Reprojection.vert",
	                                     "Reprojection.frag", false);

	auto displayShader = new Shader("Reprojection.vert",
	                                "DisplayShader.frag", false);

	overlayShader = new Shader("Reprojection.vert",
	                           "OverlayShader.frag", false);

	//Create FBO and texture

	uint blurFBO, blurFBOTexture;

	//Set up framebuffer for blurring
	glGenFramebuffers(1, &blurFBO);
	glBindFramebuffer(GL_FRAMEBUFFER, blurFBO);
	//Create a texture
	glGenTextures(1, &blurFBOTexture);
	glBindTexture(GL_TEXTURE_2D, blurFBOTexture);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA32F, RENDER_WIDTH, RENDER_HEIGHT, 0, GL_RGBA, GL_FLOAT, NULL);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
	//Bind the texture
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, blurFBOTexture, 0);

	GLenum status = glCheckFramebufferStatus(GL_FRAMEBUFFER);
	if (status != GL_FRAMEBUFFER_COMPLETE)
	{
		printf("Framebuffer is not complete! Status: %d \n", status);
	}

	uint secondBlurFBO, secondBlurFBOTexture;

	//Set up framebuffer for blurring
	glGenFramebuffers(1, &secondBlurFBO);
	glBindFramebuffer(GL_FRAMEBUFFER, secondBlurFBO);
	//Create a texture
	glGenTextures(1, &secondBlurFBOTexture);
	glBindTexture(GL_TEXTURE_2D, secondBlurFBOTexture);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA32F, RENDER_WIDTH, RENDER_HEIGHT, 0, GL_RGBA, GL_FLOAT, NULL);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
	//Bind the texture
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, secondBlurFBOTexture, 0);
	...
	uint dofFBO, dofFBOTexture;

	//Set up framebuffer for blurring
	glGenFramebuffers(1, &dofFBO);
	glBindFramebuffer(GL_FRAMEBUFFER, dofFBO);
	//Create a texture
	glGenTextures(1, &dofFBOTexture);
	glBindTexture(GL_TEXTURE_2D, dofFBOTexture);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA32F, RENDER_WIDTH, RENDER_HEIGHT, 0, GL_RGBA, GL_FLOAT, NULL);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
	//Bind the texture
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, dofFBOTexture, 0);
	...
	glGenFramebuffers(1, &secondDofFBO);
	glBindFramebuffer(GL_FRAMEBUFFER, secondDofFBO);
	//Create a texture
	glGenTextures(1, &secondDofFBOTexture);
	glBindTexture(GL_TEXTURE_2D, secondDofFBOTexture);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA32F, RENDER_WIDTH, RENDER_HEIGHT, 0, GL_RGBA, GL_FLOAT, NULL);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
	//Bind the texture
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, secondDofFBOTexture, 0);
	...
	uint reprojectedColorFBO, reprojectedColorFBOTexture;

	glGenFramebuffers(1, &reprojectedColorFBO);
	glBindFramebuffer(GL_FRAMEBUFFER, reprojectedColorFBO);
	//Create a texture
	glGenTextures(1, &reprojectedColorFBOTexture);
	glBindTexture(GL_TEXTURE_2D, reprojectedColorFBOTexture);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA32F, RENDER_WIDTH, RENDER_HEIGHT, 0, GL_RGBA, GL_FLOAT, NULL);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
	...
	//Bind the texture
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, reprojectedColorFBOTexture, 0);
	...
	uint reprojectedLightFBO, reprojectedLightFBOTexture;

	glGenFramebuffers(1, &reprojectedLightFBO);
	glBindFramebuffer(GL_FRAMEBUFFER, reprojectedLightFBO);
	//Create a texture
	glGenTextures(1, &reprojectedLightFBOTexture);
	glBindTexture(GL_TEXTURE_2D, reprojectedLightFBOTexture);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA32F, RENDER_WIDTH, RENDER_HEIGHT, 0, GL_RGBA, GL_FLOAT, NULL);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
	//Bind the texture
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, reprojectedLightFBOTexture, 0);
	...
	//Create a texture
	glGenTextures(1, &renderTargetBuffer->ID);
	glBindTexture(GL_TEXTURE_2D, renderTargetBuffer->ID);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA32F, RENDER_WIDTH, RENDER_HEIGHT, 0, GL_RGBA, GL_FLOAT, NULL);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
	//Bind the texture
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, renderTargetBuffer->ID, 0);
	...
	uint displayFBO, displayFBOTexture;

	glGenFramebuffers(1, &displayFBO);
	glBindFramebuffer(GL_FRAMEBUFFER, displayFBO);
	//Create a texture
	glGenTextures(1, &displayFBOTexture);
	glBindTexture(GL_TEXTURE_2D, displayFBOTexture);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA32F, RENDER_WIDTH, RENDER_HEIGHT, 0, GL_RGBA, GL_FLOAT, NULL);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
	//Bind the texture
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, displayFBOTexture, 0);
	...
	uint overlayFBO, overlayFBOTexture;
	glGenFramebuffers(1, &overlayFBO);
	glBindFramebuffer(GL_FRAMEBUFFER, overlayFBO);
	//Create a texture
	glGenTextures(1, &overlayFBOTexture);
	glBindTexture(GL_TEXTURE_2D, overlayFBOTexture);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA32F, RENDER_WIDTH, RENDER_HEIGHT, 0, GL_RGBA, GL_FLOAT, NULL);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
	//Bind the texture
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, overlayFBOTexture, 0);
...
#pragma endregion
	...
	app->renderTexture = reprojectedColorFBOTexture;
	app->debugRenderTexture = renderTargetDebug->ID;


	//Draw loading screen
	overlay->Clear(float4(0.f, 0.f, 0.1f, 1.f));
	overlay->Print("LOADING...", (UI_WIDTH / 2) - 26, (UI_HEIGHT / 2) - 3, float4(1.f, 0, 0, 1.f),
	               float4(0.f, 0.f, 0.f, 1.f));
	if (app->overlay) overlayBuffer->CopyFrom(app->overlay);
	overlayShader->Bind();
	glBindFramebuffer(GL_FRAMEBUFFER, 0);
	...
	tracerThread = new std::thread(TracerThread);

	int CursorLock = GLFW_CURSOR_DISABLED;
	...
	while (!glfwWindowShouldClose(window) && !app->m_quit)
	{
		if (startedOnce)
		{
			//There is memory leaks here but it doesnt really matter

			std::unique_lock<std::mutex> lock(surfaceMutex);
			frameReady.wait(lock, [] { return frameIsReady; }); // Make sure were not rendering or something

			AudioManager::GetAudioManager().StopAll();
			app->Shutdown();
			delete dynamic_cast<Renderer*>(app);
			app = new Renderer();
			app->screen = screen;
			app->debugScreen = debugScreen;
			app->normals = normals;
		...
		while (app->m_running && !app->m_quit && running && !glfwWindowShouldClose(window))
		{
			if (app->m_cursorLockMode != CursorLock)
			{
				CursorLock = app->m_cursorLockMode;
				glfwSetInputMode(window, GLFW_CURSOR, CursorLock);
			}

			{
				std::unique_lock<std::mutex> lock(surfaceMutex);
				frameReady.wait(lock, [] { return frameIsReady; });
				if (app->screen) renderTarget->CopyFrom(app->screen);
				if (app->screen) renderTargetLightBuffer->CopyTo(app->screen);
				if (app->screenLighting) renderTargetLight->CopyFrom(app->screenLighting);
				if (app->normals) renderNormals->CopyFrom(app->normals);
				if (app->positions) renderPos->CopyFrom(app->positions);
				if (app->debugScreen) renderTargetDebug->CopyFrom(app->debugScreen);
				if (app->overlay) overlayBuffer->CopyFrom(app->overlay);
				frameIsReady = false;
				// performance report - running average - ms, MRays/s
				float currentFrameTime = timer.elapsed();
				avg = (1 - alpha) * avg + alpha * currentFrameTime * 1000.f;
				if (alpha > 0.05f) alpha *= 0.5f;
				app->m_fps = 1000.0f / avg;
				app->m_avg = avg;
				timer.reset();
			}
			frameReady.notify_one();
			{
				...
				if (frameNr++ > 1)
				{
					...
					//This is a mess of different passes and buffers and only god knows what all the textures buffers and variables mean

					glBindFramebuffer(GL_FRAMEBUFFER, reprojectedLightFBO);
					reprojectionShader->Bind();
					reprojectionShader->SetInputTexture(0, "c", renderTargetLight);
					reprojectionShader->SetInputTexture(2, "N", renderNormals);
					reprojectionShader->SetInputTexture(3, "P", renderPos);
					reprojectionShader->SetInputTexture(4, "oldC", renderTargetLightBuffer);
					reprojectionShader->SetInputTexture(5, "oldN", renderNormalsBuffer);
					reprojectionShader->SetInputTexture(6, "oldP", renderPosBuffer);
					reprojectionShader->SetInt("resolutionX", RENDER_WIDTH);
					reprojectionShader->SetInt("resolutionY", RENDER_HEIGHT);
					reprojectionShader->SetFloat("pixelChangeTreshold", app->pixelChangeTreshold);
					reprojectionShader->SetUInt("reproject", app->reproject);
					reprojectionShader->SetFloat("neighBourSimilarityTreshold", app->reprojectSimilarityTreshold);
					reprojectionShader->SetFloat("maximumMipMap", 10.f);
					reprojectionShader->SetFloat("lerp", app->accumulatorLerp);
					reprojectionShader->SetFloat("lerpDiff", app->accumulatorLerpDiff);
					reprojectionShader->SetUInt("forceAccumulate", app->forceAccumulate);
					reprojectionShader->SetUInt("forceUpdate", app->forceUpdate);

					DrawQuad();
					reprojectionShader->Unbind();


					glBindFramebuffer(GL_FRAMEBUFFER, blurFBO);
					blurShader->Bind();
					blurShader->SetUInt("Flip", false);

					blurShader->SetInputTexture(0, "c", reprojectedLightFBOTexture);
					blurShader->SetInputTexture(1, "n", renderNormals);
					blurShader->SetInputTexture(2, "p", renderPos);
					blurShader->SetInt("resolutionX", RENDER_WIDTH);
					blurShader->SetInt("resolutionY", RENDER_HEIGHT);
					blurShader->SetUInt("horizontal", true);
					blurShader->SetFloat("edgeFallSpeed", app->edgeFallSpeed);
					blurShader->SetFloat("posFallSpeed", app->posFallSpeed);
					DrawQuad();
					blurShader->Unbind();
					glBindFramebuffer(GL_FRAMEBUFFER, secondBlurFBO);
					blurShader->Bind();
					...
					blurShader->SetInputTexture(0, "c", blurFBOTexture);
					blurShader->SetInputTexture(1, "n", renderNormals);
					blurShader->SetInputTexture(2, "p", renderPos);
					blurShader->SetInt("resolutionX", RENDER_WIDTH);
					blurShader->SetInt("resolutionY", RENDER_HEIGHT);
					blurShader->SetUInt("horizontal", false); //vertical
					blurShader->SetFloat("edgeFallSpeed", app->edgeFallSpeed);
					blurShader->SetFloat("posFallSpeed", app->posFallSpeed);
					DrawQuad();
					blurShader->Unbind();
					...
					glBindFramebuffer(GL_FRAMEBUFFER, displayFBO);
					displayShader->Bind();
					displayShader->SetInputTexture(0, "c", renderTarget);
					if (app->denoise)
						displayShader->SetInputTexture(1, "l", secondBlurFBOTexture);
					else
						displayShader->SetInputTexture(1, "l", reprojectedLightFBOTexture);
					displayShader->SetInt("resolutionX", RENDER_WIDTH);
					displayShader->SetInt("resolutionY", RENDER_HEIGHT);
					displayShader->SetUInt("tonemap", app->tonemap);
					displayShader->SetFloat("exposure", app->exposure);

					displayShader->SetUInt("vignetting", app->vignetting);
					displayShader->SetFloat("vignetteSize", app->vignetteSize);
					displayShader->SetFloat("vignetteSmooth", app->vignetteSmooth);

					displayShader->SetUInt("abberation", app->abberation);
					displayShader->SetFloat("distortion", app->distortion);

					displayShader->SetUInt("scanLines", app->scanLines);
					displayShader->SetUInt("uvDistort", app->CRTDistort);
					...
					glBindFramebuffer(GL_FRAMEBUFFER, dofFBO);
					dofShader->Bind();
					dofShader->SetUInt("Flip", false);
					dofShader->SetInputTexture(0, "c", displayFBOTexture);
					dofShader->SetInputTexture(1, "n", renderNormals);
					dofShader->SetInputTexture(2, "p", renderPos);
					dofShader->SetInt("resolutionX", RENDER_WIDTH);
					dofShader->SetInt("resolutionY", RENDER_HEIGHT);
					dofShader->SetUInt("horizontal", true);
					dofShader->SetFloat("amount", app->postDofAmount);
					dofShader->SetFloat("fallSpeed", app->postDofFalloff);
					dofShader->SetFloat("focusDistance", app->postDofFocusDist);
					DrawQuad();
					dofShader->Unbind();
					...
					dofShader->SetInputTexture(0, "c", dofFBOTexture);
					dofShader->SetInputTexture(1, "n", renderNormals);
					dofShader->SetInputTexture(2, "p", renderPos);
					dofShader->SetInt("resolutionX", RENDER_WIDTH);
					dofShader->SetInt("resolutionY", RENDER_HEIGHT);
					dofShader->SetUInt("horizontal", false); //vertical
					dofShader->SetFloat("amount", app->postDofAmount);
					dofShader->SetFloat("fallSpeed", app->postDofFalloff);
					dofShader->SetFloat("focusDistance", app->postDofFocusDist);
...
#ifdef DEBUG_UI
					glBindFramebuffer(GL_FRAMEBUFFER, overlayFBO);
					overlayShader->SetUInt("Flip", false);
#else
					glBindFramebuffer(GL_FRAMEBUFFER, 0);
					glViewport(0, 0, WINDOW_WIDTH, WINDOW_HEIGTH);
					glClearColor(0.1f, 0.0f, 0.0f, 1.0f);
					...
					if (actualAspect < ASPECT_RATIO)
					{
						int newHeight = static_cast<int>(WINDOW_WIDTH / ASPECT_RATIO);
						int yOffset = (WINDOW_HEIGTH - newHeight) / 2;
						glViewport(0, yOffset, WINDOW_WIDTH, newHeight);
					}
					else if (actualAspect > ASPECT_RATIO)
					{
						int newWidth = static_cast<int>(WINDOW_HEIGTH * ASPECT_RATIO);
						int xOffset = (WINDOW_WIDTH - newWidth) / 2;
						glViewport(xOffset, 0, newWidth, WINDOW_HEIGTH);
					}
					overlayShader->SetUInt("Flip", true);
#endif
					overlayShader->SetInputTexture(0, "c", secondDofFBOTexture);
					...
					//Copy to reprojection buffer
					glBindFramebuffer(GL_FRAMEBUFFER, copyFBO);
					glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, renderTargetBuffer->ID,
					                       0);

					copyShader->Bind();
					copyShader->SetInputTexture(0, "c", reprojectedColorFBOTexture); // copy before post processing
					DrawQuad();

					glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D,
					                       renderTargetLightBuffer->ID,
					                       0);
					copyShader->SetInputTexture(0, "c", reprojectedLightFBOTexture);
					DrawQuad();

					glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, renderNormalsBuffer->ID,
					                       0);
					copyShader->SetInputTexture(0, "c", renderNormals);
					DrawQuad();

					glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, renderPosBuffer->ID, 0);
					copyShader->SetInputTexture(0, "c", renderPos);
					DrawQuad();
					...
					glBindTexture(GL_TEXTURE_2D, renderNormalsBuffer->ID);
					glGenerateMipmap(GL_TEXTURE_2D);

					glBindTexture(GL_TEXTURE_2D, renderPosBuffer->ID);
					glGenerateMipmap(GL_TEXTURE_2D);
					...
					app->DisplayImage("Normals", renderNormals->ID, false);
					//app->DisplayImage("Positions", renderPosBuffer->ID, false); // cant really display these since alpha is mostly 0
					app->DisplayImage("Color", renderTarget->ID);
					app->DisplayImage("Raw Light Data", renderTargetLight->ID);
					//app->DisplayImage("Reprojected Color", reprojectedColorFBOTexture); // not using since color has no noise
					app->DisplayImage("Reprojected Light", reprojectedLightFBOTexture);
					app->DisplayImage("Denoised Light data 1", blurFBOTexture, false);
					app->DisplayImage("Denoised Light data 2", secondBlurFBOTexture, false);
					app->DisplayImage("Combined + Post process", displayFBOTexture);
					app->DisplayImage("Final Render", overlayFBOTexture, false, true);
					if (app->uiUpdated)
					{
						ImGui::Render();
...
static int glad_gl_get_extensions(const char** out_exts, char*** out_exts_i)
{
	...
	if (glad_glGetStringi != NULL && glad_glGetIntegerv != NULL)
	{
		...
		{
			auto gl_str_tmp = (const char*)glad_glGetStringi(GL_EXTENSIONS, index);
			size_t len = strlen(gl_str_tmp) + 1;

			auto local_str = (char*)malloc(len * sizeof(char));
			if (local_str == NULL)
			{
				exts_i[index] = NULL;
...
using GLADglprocaddrfunc = void* (GLAD_API_PTR*)(const char*);
