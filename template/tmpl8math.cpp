...
__m128i InitSeed(const __m128i _seedBase)
{
	static const __m128i one = _mm_set1_epi32(1);
	static const __m128i seventeen = _mm_set1_epi32(17);
	static const __m128i sixtyOne = _mm_set1_epi32(61);
	static const __m128i nine = _mm_set1_epi32(9);
	static const __m128i largeNum = _mm_set1_epi32(0x27d4eb2d);

	__m128i s = _mm_mul_epi32(_mm_add_epi32(_seedBase, one), seventeen);

	s = _mm_xor_epi32(_mm_xor_epi32(s, sixtyOne), _mm_bsrli_si128(s, 16));
	s = _mm_mul_epi32(s, nine);
	s = _mm_xor_epi32(s, _mm_bsrli_si128(s, 4));
	s = _mm_mul_epi32(s, largeNum);
	s = _mm_xor_epi32(s, _mm_bsrli_si128(s, 15));

	return s;
}

__m128i RandomUInt(__m128i& _seed)
{
	_seed = _mm_xor_epi32(_seed, _mm_bslli_si128(_seed, 13));
	_seed = _mm_xor_epi32(_seed, _mm_bsrli_si128(_seed, 17));
	_seed = _mm_xor_epi32(_seed, _mm_bslli_si128(_seed, 5));
	return _seed;
}

__m128 RandomFloat(__m128i& _seed)
{
	static const __m128 intToFloat = _mm_set1_ps(2.3283064365387e-10f);
	return _mm_mul_ps(_mm_cvtepi32_ps(_seed), intToFloat);
}

float3 RandomUnitVector(uint& _seed)
{
	auto vec = float3(RandomFloat(_seed) - 0.5f, RandomFloat(_seed) - 0.5f, RandomFloat(_seed) - 0.5f);;
	float len2 = length2(vec);
	while (len2 < 0.0001f)
	{
		vec = float3(RandomFloat(_seed) - 0.5f, RandomFloat(_seed) - 0.5f, RandomFloat(_seed) - 0.5f);
		len2 = length2(vec);
	}
	return vec / sqrt(len2);
}

float3 CosineRandomUnitVectorHemisphere(uint& _seed, float3 normal)
{
	float3 dir = normal + RandomUnitVector(_seed);
	float len2 = length2(dir);
	while (len2 < 0.0001f)
	{
		dir = normal + RandomUnitVector(_seed);
		len2 = length2(dir);
	}
	return dir / sqrt(len2);
}

float3 RandomUnitVectorCircle(uint& _seed)
{
	float r = sqrt(RandomFloat(_seed));
	float theta = RandomFloat(_seed) * 2 * PI;
	float x = r * cos(theta);
	float y = r * sin(theta);
	return float3(x, y, 0);
}
...
float half_to_float(const half x)
{
	...
	return as_float(
		(x & 0x8000) << 16 | (e != 0) * ((e + 112) << 23 | m) | ((e == 0) & (m != 0)) * ((v - 37) << 23 | ((m << (150 -
			v)) & 0x007FE000)));
}
...
half float_to_half(const float x)
{
	...
	return (half)((b & 0x80000000) >> 16 | (e > 112) * ((((e - 112) << 10) & 0x7C00) | m >> 13) | ((e < 113) & (e >
		101)) * ((((0x007FF000 + m) >> (125 - e)) + 1) >> 1) | (e > 143) * 0x7FFF);
	// sign : normalized : denormalized : saturate
}

quat mat4ToQuaternion(const mat4& mat)
{
	quat rot;

	// Step 1: Extract scale from matrix
	float sx = sqrt(mat[0] * mat[0] + mat[4] * mat[4] + mat[8] * mat[8]);
	float sy = sqrt(mat[1] * mat[1] + mat[5] * mat[5] + mat[9] * mat[9]);
	//float sz = sqrt(mat[2] * mat[2] + mat[6] * mat[6] + mat[10] * mat[10]);
	...
	// Step 2: Normalize the rotation matrix (remove scale)
	float m00 = mat[0] / sx, m01 = mat[4] / sx, m02 = mat[8] / sy;
	float m10 = mat[1] / sx, m11 = mat[5] / sx, m12 = mat[9] / sy;
	float m20 = mat[2] / sx, m21 = mat[6] / sx, m22 = mat[10] / sy;


	// Step 3: Convert to quaternion
	float trace = m00 + m11 + m22; // Sum of diagonal elements
	if (trace > 0.0f)
	{
